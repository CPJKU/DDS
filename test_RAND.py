...
from data import MAPS_ISOL_NoteFrames, MAPS_ChordsFramesFactory, SingleNoteTimbreFramesFactory
...
def get_arguments():
    ...
    parser = argparse.ArgumentParser(
        description='Evaluates specified method on specified subset of MAPS/RAND.')
    parser.add_argument(
    ...
    parser.add_argument(
        '--intensity', type=str, required=True,
        help='Intensity range. Options are [I60-68, I32-96].')
    parser.add_argument(
        '--polyphony', type=str, required=True,
        help='Polyphony level. Options are [P2, P3, P4, P5, P6, P7].')
    parser.add_argument(
    ...
    parser.add_argument(
        '--csvfile', type=str, default='results_grid_MAPS_RAND',
...
def main():
    ...
    assert args.intensity in ['I60-68', 'I32-96']
    assert args.polyphony in ['P2', 'P3', 'P4', 'P5', 'P6', 'P7']
    ...
    # get MAPS/RAND data factory
    subset = 'RAND'
    key_seq = ['M36-95', args.intensity, args.polyphony]

    chords_frames_factory = MAPS_ChordsFramesFactory(subset, args.instr,
        sr=hp.sr, n_fft=hp.n_fft, hop_length=hp.hop_length,
        log_str=hp.log_str, keep_bins=hp.keep_bins)

    # produce data to evaluate on
    S = chords_frames_factory.get_frames(key_seq)
    pianoroll = chords_frames_factory.get_pianoroll(key_seq)
    if args.max_frames:
        S = S[:, :args.max_frames]
        pianoroll = pianoroll[:, :args.max_frames]

    # determine log path for this config
    cfg = f'{subset}_{chords_frames_factory.instrument}'
    key_seq_str = '_'.join(key_seq)
    logdir = f'logs/sede/{args.hp}/{args.train_set}/{cfg}/{key_seq_str}/{args.method}'

    # extract used note range
    note_range_low, note_range_high = [int(n) for n in key_seq[0].replace('M', '').split('-')]
    ...
    plt.title(f'{args.method} | {subset} | {chords_frames_factory.instrument} | ' + ' | '.join(key_seq))
    ...
    log_line = f'{args.method} {args.hp} {args.train_set} MAPS {chords_frames_factory.instrument} RAND M36-95 ' + \
               f'{args.intensity} {args.polyphony} {args.max_iter} {args.max_frames} {args.n_cps} {args.dds_step_size} ' + \
               f'{gCAR:.5f} {mfCAR:.5f} ' + \
